...
//
// The whole point of the spike is to find out whether a Windows exec-proxy can be BOTH correct (exit codes,
// Ctrl-C, .cmd targets, inherited handles) and fast enough to sit on every tool call. See docs/m1-spike-findings.md.
//
// Resolution here is deliberately minimal (M1 hard-registers one tool per the plan; full precedence,
// tack.yml walk-up and central bindings are M2). resolved.json schema for the spike:
...
//
// Windows sharp edges this spike exists to nail down:
...
//    ("not a valid application"). Such targets must go through cmd.exe /c. The scope-plan's note that a
//    full path to a .cmd "just resolves via the associated interpreter" only holds for UseShellExecute=true,
//    which spawns a new window and breaks handle inheritance -- so we route .cmd/.bat via cmd.exe here.
//  * No redirection + UseShellExecute=false => the child inherits our stdin/stdout/stderr and console,
//    so a caller that piped us gets the child's output directly. No new window.
