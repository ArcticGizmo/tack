...
#      runtime init + JSON read), which is the part tack controls and the part AOT is meant to shrink.
#
...
#
# ASCII only, no BOM (perch install-script lesson: PS 5.1 mis-parses non-ASCII).
...
# $PSScriptRoot can be empty depending on how the script is launched; fall back to the invocation path.
...
# which terminates regardless of this preference.
...
try {
    # Lay a shim dir out the way production would: copy the shim payload, then copies named per tool.
    ...
    # A trivial target: cheap .cmd that just exits 0 (node/npm are frequently .cmd on Windows anyway).
    ...
    $json = @{ tools = @{ node = @{ default = $targetCmd } } } | ConvertTo-Json -Depth 6
    $json | Out-File -FilePath $resolved -Encoding ascii
    ...
    $shimFixed   = Measure-Call $ghostExe ''                          # A: shim self-cost (no child)
    $throughShim = Measure-Call $nodeExe  ''                          # B1: shim -> .cmd
    $direct      = Measure-Call $env:ComSpec ('/c "' + $targetCmd + '"') # B2: .cmd directly
